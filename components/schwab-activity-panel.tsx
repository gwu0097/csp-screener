...
// Review feed for schwab_account_transactions rows the poller has
// touched but the user hasn't dismissed yet — both what it applied
// automatically ("applied": submitted/expired/assigned, shown so the
// user can validate what changed, e.g. a same-day roll) and what it
// couldn't apply ("needs_review": no matching position, a bulk-create
// error, or a suspected duplicate it declined to write — see
// lib/schwab-account-import.ts's own comment on why skipped_duplicate
// belongs here rather than being silently dropped). Purely
// administrative rows (dividends, cash transfers, fee-only legs)
// never appear — they're excluded server-side. Persistent panel, not
// a toast: this can land from an unattended cron run with nobody
// watching.
...
type Props = {
  onImport: (item: ActivityItem) => void;
...
export function SchwabActivityPanel({ onImport, onImportAll, refreshToken }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch("/api/schwab-account/activity", { cache: "no-store" })
      .then((r) => (r.ok ? (r.json() as Promise<{ items: ActivityItem[] }>) : null))
      .then((j) => {
  ...
  async function dismissOne(id: string, reason: string) {
    const res = await fetch(`/api/schwab-account/activity/${id}/dismiss`, {
      method: "POST",
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className={"text-base font-bold " + (alarmed ? "text-amber-200" : "text-emerald-200")}>
            Schwab activity: {items.length} to review
          </span>
        </div>
      ...
      <p className={"mt-1 text-xs " + (alarmed ? "text-amber-200/80" : "text-emerald-200/80")}>
        What the Schwab auto-import poller applied and what it couldn&apos;t. Applied rows are already
