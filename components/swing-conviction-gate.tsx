...
export function SwingConvictionGate({
  ...
  // Pull the latest valuation model for this symbol so the user can see
  // bear/base/bull targets while writing their thesis. Display-only — we
  // don't currently persist a target price on the idea row.
    ...
    (async () => {
      try {
        const res = await fetch(
          `/api/research/${encodeURIComponent(idea.symbol)}/valuation`,
          { cache: "no-store" },
        );
        if (!res.ok) return;
        const json = (await res.json()) as {
          versions?: Array<{
            output: {
              outputs: {
                bear: { price_target: number };
                base: { price_target: number };
                bull: { price_target: number };
        ...
        setValuation({
          bear: latest.output.outputs.bear.price_target,
          base: latest.output.outputs.base.price_target,
          bull: latest.output.outputs.bull.price_target,
          weighted: latest.output.outputs.weighted_target,
        });
      } catch {
        /* swallow — valuation is optional context here */
      }
    })();
    return () => {
      cancelled = true;
        ...
        <div className="space-y-4 py-2 text-sm">
          {valuation && (
            <div className="rounded-md border border-emerald-500/30 bg-emerald-500/[0.05] p-2 text-xs">
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-0.5 font-mono text-foreground">
                <span>Bear ${Math.round(valuation.bear)}</span>
                <span>Base ${Math.round(valuation.base)}</span>
                <span>Bull ${Math.round(valuation.bull)}</span>
                <span className="text-emerald-300">
                  Weighted ${Math.round(valuation.weighted)}
                </span>
              </div>
            </div>
          )}
