// Bulk trade import — match-or-create positions, insert fills,
// recompute aggregates, duplicate detection, campaign/chain
// stamping. Lives in lib/ (not app/api/trades/bulk-create/route.ts)
// because Next.js's App Router only allows specific recognized names
// (GET/POST/etc + a small set of config exports) to be exported from
// a route.ts file — exporting this as a plain function from the route
// file itself passed `tsc --noEmit` and `next lint` locally but failed
// `next build` on Vercel ("is not a valid Route export field"), which
// silently blocked every deploy behind it for hours. The route file
// is now a thin wrapper: derive userId from the session, parse the
// body, call runBulkCreate.
