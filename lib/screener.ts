...
export type PersonalHistory = {
  ...
  swingCount?: number;
  // A speculative long-option bet (a "lottery ticket" or a longer-dated
  // directional call bought purely to see if it hits) — not a strategy,
  // excluded from CSP grading the same way, tracked in its own bucket
  // so it doesn't blur into swing/recovery counts (2026-09-11).
  speculativeCount?: number;
...
const TRADE_TYPE_SEVERITY: Record<string, number> = { clean: 0, rolled: 1, recovery_play: 2, swing: 3, speculative: 4 };
...
type PersonalStats = {
  ...
  swingCount: number;
  // A speculative long-option bet — a lottery ticket or a longer-dated
  // directional call, not a strategy. Excluded from CSP grading like
  // recovery_play/swing, counted separately (2026-09-11).
  speculativeCount: number;
};
...
function personalStats(rows: PersonalRow[]): PersonalStats {
  ...
  return { tradeCount, winRate, avgRoc, dropWinRate, cleanCount, rolledCount, recoveryCount, swingCount, speculativeCount };
}
...
export async function getPersonalHistory(
  ...
  try {
    ...
    const tickerLevel = {
      ...
      speculative: ticker.speculativeCount,
    };
    const sampleWeight =
        ...
        if (!secRes.error) {
          ...
          if (sec.tradeCount >= 10) {
            sector = {
              ...
              speculativeCount: sec.speculativeCount,
            };
            sectorStatsFull = sec;
          }
        }
    ...
    if (sector && sectorStatsFull) {
      return {
        ...
        speculativeCount: sector.speculativeCount,
        dataInsufficient: false,
        scope: "sector",
        sectorIndustry: industry,
        tickerLevel,
...
export function calculateThreeLayerGrade(
  ...
  if (!history.dataInsufficient && history.winRate !== null) {
    ...
    if (historyScope === "sector") {
      ...
      const tickerBit = t
        ? `${t.clean} clean campaign${t.clean === 1 ? "" : "s"} on this ticker (need 5+)${t.recovery > 0 ? `, ${t.recovery} recovery play${t.recovery === 1 ? "" : "s"} excluded from CSP grading` : ""}${t.swing > 0 ? `, ${t.swing} swing trade${t.swing === 1 ? "" : "s"} excluded from CSP grading` : ""}${t.speculative > 0 ? `, ${t.speculative} speculative trade${t.speculative === 1 ? "" : "s"} excluded from CSP grading` : ""}. `
        : "";
      historyLines.push(
      ...
      const breakdown =
        (history.rolledCount ?? 0) > 0 || (history.recoveryCount ?? 0) > 0 || (history.swingCount ?? 0) > 0 || (history.speculativeCount ?? 0) > 0
          ? ` [${history.cleanCount ?? history.tradeCount} clean${(history.rolledCount ?? 0) > 0 ? ` · ${history.rolledCount} rolled (half-weight)` : ""}${(history.recoveryCount ?? 0) > 0 ? ` · ${history.recoveryCount} recovery play${(history.recoveryCount ?? 0) === 1 ? "" : "s"} excluded` : ""}${(history.swingCount ?? 0) > 0 ? ` · ${history.swingCount} swing trade${(history.swingCount ?? 0) === 1 ? "" : "s"} excluded` : ""}${(history.speculativeCount ?? 0) > 0 ? ` · ${history.speculativeCount} speculative trade${(history.speculativeCount ?? 0) === 1 ? "" : "s"} excluded` : ""}]`
          : "";
      historyLines.push(
    ...
    const excluded =
      (history.recoveryCount ?? 0) > 0 || (history.swingCount ?? 0) > 0 || (history.speculativeCount ?? 0) > 0
        ? ` ${[
            ...
            (history.swingCount ?? 0) > 0
              ...
              : null,
            (history.speculativeCount ?? 0) > 0
              ? `${history.speculativeCount} speculative trade${(history.speculativeCount ?? 0) === 1 ? "" : "s"}`
              : null,
          ]
            .filter(Boolean)
  ...
  return {
    ...
    personalFactors: {
      ...
      speculativeCount: history.speculativeCount,
