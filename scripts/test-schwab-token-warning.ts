...
run("Friday before expiry weekend, exactly 24h out (clause 4 — the 24h threshold is inclusive)", la("2026-08-21", 10), expiresNextSat, refreshedSat);
run("The expiry Saturday itself, hours before expiry (clause 4 on 24h proximity)", la("2026-08-22", 8), expiresNextSat, refreshedSat);
...
// Within 24h, on a weekend expiry — clause 4 must win over clause 2.
run("Friday evening, expiry is Saturday ~18h away (clause 4 must override clause 2)", la("2026-08-21", 16), la("2026-08-22", 10), refreshedSat);
...
// clause 2 rather than clause 4 winning by proximity.
const expiresSunEvening = la("2026-08-23", 20); // Sun Aug 23, 8pm
run("Saturday morning, expiry is Sunday evening (~34h out — clause 2, not 4)", la("2026-08-22", 10), expiresSunEvening, la("2026-08-15", 10));
run("Thursday, several days before a healthy weekend expiry (should be silent)", la("2026-08-20", 10), expiresNextSat, refreshedSat);

// Clause 2's generalized anchor-Saturday rule — canonical cases from
// the request that replaced the old always-fires-on-weekday clause 2.
run("Expiry Sun 08-16, today Fri 08-14 (silent — warns from Sat 08-15)", la("2026-08-14", 10), la("2026-08-16", 11), null);
run("Expiry Mon 08-24, today Mon 08-17 (silent — warns from Sat 08-22)", la("2026-08-17", 10), la("2026-08-24", 11), null);
run("Expiry Wed 08-19, today Mon 08-17 (warns immediately — anchor Sat 08-15 already passed)", la("2026-08-17", 10), la("2026-08-19", 11), null);
